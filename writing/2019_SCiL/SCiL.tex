...
More recent work has focused on predicting guilt verdicts from the Supreme Courts in the Philippines \citep{virtucio2018predicting} and Thailand \citep{kowsrihawat2018predicting} on the basis of facts presented and law texts. \citeauthor{kowsrihawat2018predicting} argue for a recurrent neural network with attention to make these predictions and achieves \ek{accuracy} \cp{I actually wouldn't give the accuracy number, since those can't really be understood in isolation. I'd use more general language like ``is effective''}. Note that this work is not concerned with the linguistic basis of subjective moral guilt judgments, but Court guilt verdicts on the basis of a given law text. 
...
\begin{figure}[th]
  ...
  \caption{Model architecture. \cp{I would not draw the output
      ...
      and these are fed into the LSTM.}
    }
  \label{fig:model}
\end{figure}
