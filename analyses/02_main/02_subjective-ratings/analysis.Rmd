...
ggplot(df_best, aes(x=factor(generation),
                   y=sim)) +
  # geom_violin() +
  geom_point(alpha=0.2) +
...
ggsave(filename="jaccdistance.pdf",path = here("writing","2019_cogsci","graphs"),width = 5,height = 3)
...
ggplot(df_hedges_corpus,aes(x=factor(generation),
                     y=hedges_prop)) +
  # geom_violin() +
  ...
  facet_wrap(vars(condition)) +
  xlab("Generation") +
  ylab("Proportion of hedges by number of words") +
  theme(strip.background = element_rect(fill = NA, colour = NA)) +
  theme(strip.text = element_text(size=12))

ggsave(filename="hedges_points.pdf",path = here("writing","2019_cogsci","graphs"),width = 5,height = 3)
...
m3.simple = lmer(author_belief ~ condition*ResidualDist - ResidualDist  + (1|story_reproduction), data=df_model)
summary(m3.simple)
```
