...
df_lang = df_cond %>% 
  distinct(story_reproduction,generation,condition,story_title,NumOfWords,seed,repro_id) %>% 
...
### Jaccard distance for Submission
...
df_optimal = df_addedsim %>%
  mutate(firstMeanWords = ifelse(generation==0,
                                 seed,
                                 word(seed,start = 1, end = meanNumOfWords))) %>% 
  rowwise() %>% 
...
df_optimal = df_addedsim %>%
  mutate(firstMeanWords = ifelse(generation==0,
                                 ...
                                 word(seed,start = 1, end = meanNumOfWords))) %>% 
  rowwise() %>% 
  mutate(optimalJacc = calc_jacc_dist(seed,firstMeanWords)) %>% 
  ungroup() %>% 
  group_by(generation) %>% 
  mutate(meanOptJacc = mean(optimalJacc))
