...
```{r info, fig.height=11, fig.width=9, message=FALSE, warning=FALSE, include=FALSE}
...
# df_sum = df_cond %>%
#   group_by(story_reproduction,generation,story_topic,condition,trial_type) %>% 
#   summarize(response=mean(slider_val))
```
...
ggsave(filename="subj_results_byquestion.pdf",path=here("writing","2019_cogsci","graphs"), height = 4.5, width = 11)
```
...
df_sim = df_cond %>%
  ...
  distinct(story_reproduction,generation,sim,condition,seed) %>% 
  ungroup()
...
df_best = df_sim %>%
  ...
  ungroup() %>% 
  mutate(firstMeanWords = ifelse(generation==0,seed,word(seed,start = 1, end = meanNumOfWords))) %>% 
  rowwise() %>% 
  mutate(optimalJacc = stringdistmatrix(seed,firstMeanWords,method = "jaccard")) %>% 
  ungroup() %>% 
  group_by(generation,condition) %>% 
  mutate(meanOptJacc = mean(optimalJacc))
  # mutate(optimalJacc = 1-(meanNumOfWords/159))

p_jaccdistance = ggplot(df_best, aes(x=factor(generation),
                   y=sim)) +
  # geom_violin() +
  geom_point(alpha = 0.2,
             position = position_jitter(width = 0.1, height = 0),
...
msim = lm(sim ~ generation,data=df_simmodel)
summary(msim)
msimcon = lm(sim ~ generation + condition,data=df_simmodel)
summary(msimcon)
```
...
df_hedges_corpus = df_cond %>%
  distinct(story_reproduction,generation,condition) %>% 
  rowwise() %>% 
...
p_hedges = ggplot(df_hedges_corpus,aes(x=factor(generation),
                     y=hedges_prop)) +
  # geom_violin() +
...
mhedge = lm(hedges_prop ~ as.numeric(generation), data=df_hedges_corpus)
summary(mhedge)
mhedgecon = lm(hedges_prop ~ as.numeric(generation)+condition, data=df_hedges_corpus)
summary(mhedgecon)

df_hedges_ratings = df_cond %>%
  rowwise() %>% 
  mutate_at(vars(story_reproduction),funs(str_to_lower(.))) %>% 
  mutate(hedges_nr=sum(str_count(story_reproduction,hedges))) %>% 
  mutate(NumOfWords=str_count(story_reproduction, boundary("word"))) %>% 
...
# ggplot(df_hedges_ratings,aes(x=generation,
#                      y=hedges_prop)) +
#   facet_wrap(vars(condition)) +
#   # geom_point(alpha=0.2, position = position_jitter()) +
#   # geom_point(alpha=0.2) +
...
df_hedges_guilt = df_hedges_ratings %>% 
  filter(trial_type == "suspect_committedCrime")
  # filter(trial_type == "evidence")

ggplot(df_hedges_guilt,aes(x=cut_width(hedges_prop, 0.04, center = 0),
                     y=slider_val)) +
  # geom_violin() +
...
## Condition distance in generations
```{r condition_dist, echo=FALSE}
df_cond_weak = df_cond %>% 
  filter(condition == "weak evidence") %>% 
  rename(weak_reproduction = story_reproduction) %>% 
  distinct(weak_reproduction,generation,story_topic) %>% 
  arrange(generation,story_topic) %>% 
  mutate(ID=row_number())

df_cond_strong = df_cond %>%
  filter(condition == "strong evidence") %>% 
  select(story_reproduction,generation,story_topic) %>% 
  rename(strong_reproduction = story_reproduction) %>% 
  distinct(strong_reproduction,generation,story_topic)

df_cond_strong = df_cond_strong[sample(1:nrow(df_cond_strong)), ] %>% 
  arrange(generation,story_topic) %>% 
  mutate(ID=row_number()) %>% 
  rename(strong_story_topic = story_topic) %>% 
  rename(strong_generation = generation) 

df_sim = df_cond_weak %>% 
  left_join(df_cond_strong,by="ID") %>%
  rowwise() %>% 
  mutate(sim=stringdistmatrix(weak_reproduction,strong_reproduction,method = "jaccard")) 
  
ggplot(df_sim,aes(x=generation,y=sim)) +
  geom_point(alpha=0.2) +
  stat_summary(fun.y="mean",
               geom="point",
               color = "red",
...
df_model = df_cond %>% 
  ...
  mutate(hedges_prop=hedges_nr/NumOfWords) %>% 
  ungroup() %>% 
  mutate(chedges_prop=hedges_prop-mean(hedges_prop))
  # arrange(generation) %>% 
  # arrange(sim)
...
df_mj = df_model %>%
  distinct(story_reproduction,generation,condition,sim) %>%
  ungroup() %>%
  mutate(cgeneration = generation - mean(generation))
  
m.j = lmer(sim ~ cgeneration*condition + (1|story_reproduction), data=df_mj, REML=F)
summary(m.j)
...
# mh = lmer(author_belief ~ chedges_prop * condition + (1|story_reproduction), data=df_model)
# summary(mh)
# 

m = lm(hedges_prop ~ as.numeric(generation), data=df_model)
df_model$ResidualHedgProp = resid(m)

mh_all_a = lmer(author_belief ~ as.numeric(generation)*condition + ResidualHedgProp*condition  + (1|story_reproduction), data=df_model)
summary(mh_all_a)

mh_gen = lmer(author_belief ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(mh_gen)

mh_h = lmer(author_belief ~ hedges_prop*condition  + (1|story_reproduction), data=df_model)
summary(mh_h)

# mh_all **
anova(mh_gen,mh_all_a)
# mh_all *
anova(mh_h,mh_all_a)

# Distance

m_noratings = lm(sim ~ as.numeric(generation), data=df_model)
df_model$ResidualDist = resid(m)

ggplot(df_model,aes(x=fitted(m_noratings),y=(resid(m_noratings))))+
  geom_point(alpha=0.2)

md_all = lmer(author_belief ~ as.numeric(generation)*condition + ResidualDist*condition  + (1|story_reproduction), data=df_model)
summary(md_all)

md_gen = lmer(author_belief ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(md_gen)
...
m = lm(NumOfWords ~ as.numeric(generation), data=df_model)
df_model$ResidualLen = resid(m)

ml_all = lmer(author_belief ~ as.numeric(generation)*condition + ResidualLen*condition  + (1|story_reproduction), data=df_model)
summary(ml_all)

ml_gen = lmer(author_belief ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(ml_gen)
...
mh = lmer(suspect_committedCrime ~ chedges_prop * condition + (1|story_reproduction), data=df_model)
summary(mh)

# Hedges 

mh_all_b = lmer(suspect_committedCrime ~ as.numeric(generation)*condition + ResidualHedgProp*condition  + (1|story_reproduction), data=df_model)
summary(mh_all_b)

mh_gen = lmer(suspect_committedCrime ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(mh_gen)

mh_h = lmer(suspect_committedCrime ~ hedges_prop*condition  + (1|story_reproduction), data=df_model)
summary(mh_h)

# mh_all
anova(mh_all_b,mh_gen)
# mh_all
anova(mh_all,mh_h)

# Distance

md_all = lmer(suspect_committedCrime ~ as.numeric(generation)*condition + ResidualDist*condition  + (1|story_reproduction), data=df_model)
summary(md_all)

md_gen = lmer(suspect_committedCrime ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(md_gen)
...
ml_all = lmer(suspect_committedCrime ~ as.numeric(generation)*condition + ResidualLen*condition  + (1|story_reproduction), data=df_model)
summary(ml_all)

ml_gen = lmer(suspect_committedCrime ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(ml_gen)
...
mh = lmer(author_trust ~ chedges_prop * condition + (1|story_reproduction), data=df_model)
summary(mh)

# Hedges 

mh_all = lmer(author_trust ~ as.numeric(generation)*condition + ResidualHedgProp*condition  + (1|story_reproduction), data=df_model)
summary(mh_all)

mh_gen = lmer(author_trust ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(mh_gen)

mh_h = lmer(author_trust ~ hedges_prop*condition  + (1|story_reproduction), data=df_model)
summary(mh_h)
...
md_all = lmer(author_trust ~ as.numeric(generation)*condition + ResidualDist*condition  + (1|story_reproduction), data=df_model)
summary(md_all)

md_gen = lmer(author_trust ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(md_gen)
...
ml_all = lmer(author_trust ~ as.numeric(generation)*condition + ResidualLen*condition  + (1|story_reproduction), data=df_model)
summary(ml_all)

ml_gen = lmer(author_trust ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(ml_gen)
...
mh = lmer(evidence ~ chedges_prop * condition + (1|story_reproduction), data=df_model)
summary(mh)

# Hedges 

mh_all = lmer(evidence ~ as.numeric(generation)*condition + ResidualHedgProp*condition  + (1|story_reproduction), data=df_model)
summary(mh_all)

mh_gen = lmer(evidence ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(mh_gen)

mh_h = lmer(evidence ~ hedges_prop*condition  + (1|story_reproduction), data=df_model)
summary(mh_h)
...
md_all = lmer(evidence ~ as.numeric(generation)*condition + ResidualDist*condition  + (1|story_reproduction), data=df_model)
summary(md_all)

md_gen = lmer(evidence ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(md_gen)
...
ml_all = lmer(evidence ~ as.numeric(generation)*condition + ResidualLen*condition  + (1|story_reproduction), data=df_model)
summary(ml_all)

ml_gen = lmer(evidence ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(ml_gen)
...
mh = lmer(reader_emotion ~ chedges_prop * condition + (1|story_reproduction), data=df_model)
summary(mh)

# Hedges 

mh_all = lmer(reader_emotion ~ as.numeric(generation)*condition + ResidualHedgProp*condition  + (1|story_reproduction), data=df_model)
summary(mh_all)

mh_gen = lmer(reader_emotion ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(mh_gen)

mh_h = lmer(reader_emotion ~ hedges_prop*condition  + (1|story_reproduction), data=df_model)
summary(mh_h)
...
md_all = lmer(reader_emotion ~ as.numeric(generation)*condition + ResidualDist*condition  + (1|story_reproduction), data=df_model)
summary(md_all)

md_gen = lmer(reader_emotion ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(md_gen)
...
ml_all = lmer(reader_emotion ~ as.numeric(generation)*condition + ResidualLen*condition  + (1|story_reproduction), data=df_model)
summary(ml_all)

ml_gen = lmer(reader_emotion ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(ml_gen)
...
mh = lmer(story_subjectivity ~ chedges_prop * condition + (1|story_reproduction), data=df_model)
summary(mh)

# Hedges 

mh_all = lmer(story_subjectivity ~ as.numeric(generation)*condition + ResidualHedgProp*condition  + (1|story_reproduction), data=df_model)
summary(mh_all)

mh_gen = lmer(story_subjectivity ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(mh_gen)

mh_h = lmer(story_subjectivity ~ hedges_prop*condition  + (1|story_reproduction), data=df_model)
summary(mh_h)
...
md_all = lmer(story_subjectivity ~ as.numeric(generation)*condition + ResidualDist*condition  + (1|story_reproduction), data=df_model)
summary(md_all)

md_gen = lmer(story_subjectivity ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(md_gen)
...
ml_all = lmer(story_subjectivity ~ as.numeric(generation)*condition + ResidualLen*condition  + (1|story_reproduction), data=df_model)
summary(ml_all)

ml_gen = lmer(story_subjectivity ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(ml_gen)
...
mh = lmer(suspect_conviction ~ chedges_prop * condition + (1|story_reproduction), data=df_model)
summary(mh)

# Hedges 

mh_all = lmer(suspect_conviction ~ as.numeric(generation)*condition + ResidualHedgProp*condition  + (1|story_reproduction), data=df_model)
summary(mh_all)

mh_gen = lmer(suspect_conviction ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(mh_gen)

mh_h = lmer(suspect_conviction ~ hedges_prop*condition  + (1|story_reproduction), data=df_model)
summary(mh_h)
...
md_all = lmer(suspect_conviction ~ as.numeric(generation)*condition + ResidualDist*condition  + (1|story_reproduction), data=df_model)
summary(md_all)

md_gen = lmer(suspect_conviction ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(md_gen)
...
ml_all = lmer(suspect_conviction ~ as.numeric(generation)*condition + ResidualLen*condition  + (1|story_reproduction), data=df_model)
summary(ml_all)

ml_gen = lmer(suspect_conviction ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(ml_gen)
...
mh = lmer(suspect_convictionJustified ~ chedges_prop * condition + (1|story_reproduction), data=df_model)
summary(mh)

# Hedges 

mh_all = lmer(suspect_convictionJustified ~ as.numeric(generation)*condition + ResidualHedgProp*condition  + (1|story_reproduction), data=df_model)
summary(mh_all)

mh_gen = lmer(suspect_convictionJustified ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(mh_gen)

mh_h = lmer(suspect_convictionJustified ~ hedges_prop*condition  + (1|story_reproduction), data=df_model)
summary(mh_h)
...
md_all = lmer(suspect_convictionJustified ~ as.numeric(generation)*condition + ResidualDist*condition  + (1|story_reproduction), data=df_model)
summary(md_all)

md_gen = lmer(suspect_convictionJustified ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(md_gen)
...
ml_all = lmer(suspect_convictionJustified ~ as.numeric(generation)*condition + ResidualLen*condition  + (1|story_reproduction), data=df_model)
summary(ml_all)

ml_gen = lmer(suspect_convictionJustified ~ as.numeric(generation)*condition  + (1|story_reproduction), data=df_model)
summary(ml_gen)
...
```{r}
# p_hedg_s = ggplot(df_model,aes(x=hedges_prop, y=suspect_committedCrime, color=condition)) +
#   # geom_point() +
#   geom_smooth(method="lm") +
#   ylim(0,100)
# 
# p_hedg_a = ggplot(df_model,aes(x=hedges_prop, y=author_belief, color=condition)) +
#   # geom_point() +
#   geom_smooth(method="lm") +
#   ylim(0,100)
# 
# p_sim_s = ggplot(df_model,aes(x=sim, y=suspect_committedCrime, color=condition)) +
#   # geom_point() +
#   geom_smooth(method="lm") +
#   ylim(0,100)
# 
# p_sim_a = ggplot(df_model,aes(x=sim, y=author_belief, color=condition)) +
#   # geom_point() +
#   geom_smooth(method="lm") +
#   ylim(0,100)
# 
# p_len_s = ggplot(df_model,aes(x=NumOfWords, y=suspect_committedCrime, color=condition)) +
#   # geom_point() +
#   geom_smooth(method="lm") +
#   ylim(0,100)
# 
# p_len_a = ggplot(df_model,aes(x=NumOfWords, y=author_belief, color=condition)) +
#   # geom_point() +
#   geom_smooth(method="lm") +
#   ylim(0,100)
# 
# blub = plot_grid(p_hedg_s,p_hedg_a,p_sim_s,p_sim_a,p_len_s,p_len_a, labels = "AUTO", ncol = 2, align = 'v')

df_modelplot = df_model %>%
  select(hedges_prop,sim,NumOfWords,author_belief,suspect_committedCrime,condition) %>% 
  gather(question,response,author_belief,suspect_committedCrime) %>% 
  gather(lingmetric,lingvalue,hedges_prop,sim,NumOfWords)

ggplot(df_modelplot,aes(x=lingvalue,y=response,color=condition,linetype=question)) +
  # geom_point() +
  geom_smooth(method="lm") +
  facet_wrap(vars(lingmetric),ncol = 1,scales = "free_x")
```

```{r}
m = lm(sim ~ as.numeric(generation), data=df_model)
df_model$ResidualDist = resid(m)
m = lm(NumOfWords ~ as.numeric(generation), data=df_model)
df_model$ResidualLen = resid(m)
m = lm(hedges_prop ~ as.numeric(generation), data=df_model)
...
```{r}
m1.simple = lmer(author_belief ~ condition*ResidualHedgProp - ResidualHedgProp  + (1|story_reproduction), data=df_model)
summary(m1.simple)

m2.simple = lmer(author_belief ~ condition*ResidualLen - ResidualLen  + (1|story_reproduction), data=df_model)
summary(m2.simple)

m3.simple = lmer(suspect_committedCrime ~ condition*ResidualDist - ResidualDist  + (1|story_reproduction), data=df_model)
summary(m3.simple)
```
