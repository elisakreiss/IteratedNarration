...
```{r judgments suspect guilt only, echo=FALSE}
df_suspguilt = df_reformatted %>% 
  filter(trial_type == "suspect guilt") 
...
ggplot(df_suspguilt,
  ...
  xlab("Generation") +
  ylab("Average slider rating") +
  scale_color_manual(values=c(color_strongc,color_weakc)) +
  theme(legend.position = "none") +
  theme(strip.background = element_rect(fill = NA, colour = NA))

# ggsave(filename="subjguilt_byquestion.pdf",path=here(), height = 4, width = 7)
```

```{r judgments suspect guilt only in paper, echo=FALSE}
df_suspguilt = df_reformatted %>% 
  filter(trial_type == "suspect guilt") %>% 
  mutate_at(vars(slider_val),funs(./100)) %>% 
  group_by(chain,generation,story_title,condition) %>%
  summarize(story_mean=mean(slider_val)) %>%
  ungroup() %>%
  mutate(x = row_number())
  # mutate(story_mean=mean(slider_val)) %>% 
  # ungroup()
...
ggplot(df_suspguilt, aes(x=story_mean, fill=condition)) +
  geom_histogram() +
  ylab("Number of crime stories") +
  xlab("Mean slider rating") +
  scale_fill_manual(values=c(color_strongc,color_weakc)) +
  scale_color_manual(values=c("black","black")) +
  theme(legend.position = "top") +
  xlim(0,1)

ggplot(df_suspguilt, aes(x=story_mean, color=condition)) +
...
ggsave(filename="subjguilt.pdf",path=here(), height = 3, width = 4.5)
```
...
ggsave(filename="subjguilt_byquestion.pdf",path=here(), height = 4, width = 5)
```
...
ggsave(filename="subj_results_2rows.pdf",path=here("writing","2019_cogsci","pics"), height = 5, width = 7)
```
