...
```{r}
df_stories = read_csv(here("models/01_nlu_project/tokenized_stories.csv"))
  
df_attention = read_csv(here("models/01_nlu_project/attention.csv"))

df_loss = read_csv(here("models/01_nlu_project/loss.csv")) %>% 
  arrange(loss) %>% 
...
plot= ggplot(df_plot, aes(x=xaxis,y=attention, group = 1)) +
  geom_line() +
  facet_wrap(~reordered_story_id, scales = "free") +
