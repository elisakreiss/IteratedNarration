...
random_story_ids = sort(c(19,24,22,9,20,11,16,13))
...
create_graph <- function(df_plot_slice,x_axis_size=20,angle=45) {
  ggplot(df_plot_slice, aes(x=xaxis,y=attention, group = 1)) +
    geom_line() +
    geom_text(data=distinct(df_plot_slice, reordered_story_id, .keep_all = TRUE),
              aes(x=-Inf,y=Inf,vjust=2,label=reordered_story_id),
              hjust=-1.5,
              size=10) +
    xlab("Tokenized story") +
    ...
    theme(axis.text.x = element_text(size=x_axis_size, angle = angle, hjust = 1)) +
    theme(axis.text.y = element_blank()) +
    theme(axis.ticks.y = element_blank()) +
    theme(strip.background = element_blank(),
          strip.text.x = element_blank()) +
...
g1 = create_graph(df_plot[df_plot$reordered_story_id==random_story_ids[1],])
g2 = create_graph(df_plot[df_plot$reordered_story_id==random_story_ids[2],],18,90)
g3 = create_graph(df_plot[df_plot$reordered_story_id==random_story_ids[3],])
g4 = create_graph(df_plot[df_plot$reordered_story_id==random_story_ids[4],],11,90)
g5 = create_graph(df_plot[df_plot$reordered_story_id==random_story_ids[5],])
g6 = create_graph(df_plot[df_plot$reordered_story_id==random_story_ids[6],])
g7 = create_graph(df_plot[df_plot$reordered_story_id==random_story_ids[7],])
g8 = create_graph(df_plot[df_plot$reordered_story_id==random_story_ids[8],])
...
ggsave(here("writing/2019_SCiL/graphs/attention-viz-appendix.pdf"),graph,height=30,width=20)
